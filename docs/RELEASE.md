...
The `.github/workflows/release.yml` workflow now performs that release automatically when `main` or `master` is pushed, on a matching version tag such as `v0.8`, or from **Actions > Windows Release > Run workflow**. Branch pushes publish the version declared in CMake, while tag and manual runs must match it exactly. The job uses the `windows-2022` runner required by the Visual Studio 2022 generator, builds the x64 Release executable, runs the headless protocol test (the hosted runner has no OpenGL context for the full graphical smoke test), verifies the ZIP checksum, keeps a 14-day workflow artifact, and publishes or refreshes the permanent GitHub Release assets.
...
1. Portable Windows ZIP: current target, a persistent three-round Arcade Cup, local 2v2 and rotation-based 3v3 against AI, two-camera keyboard/gamepad split-screen, configurable match formats, free training, a persistent ten-shot Target Challenge, and a checksummed release asset.
