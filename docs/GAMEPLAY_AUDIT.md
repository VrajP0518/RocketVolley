...
Audited repository versions 0.12-0.13 on September 17-25, 2026. Source inspection, executable headless tests, local hidden-window graphical smoke runs, and inspection of captured screenshots were used. This was **not hands-on keyboard/controller play-testing or a subjective audio review**. No Git commands were run. The September 23 fair-play iteration below supersedes the earlier bot shot assistance; the September 25 user play-test corrections supersede the earlier steering verdict.
...
| P1 | Left steering disagreed with directional dodging, reverse steering was inconsistent, airborne heading drifted from body yaw, and inverted cars could get stuck. | `keyboardControls`, `directionalDodge`, `driveCar`, `aiControls`. | **Corrected September 25:** the earlier world-angle assertion missed reversed driver-relative steering. Ground/aerial steering and dodges now use the driver's left/right, verified at four headings. Recovery, analog deadzone and boost fixes remain. |
...
- Split-screen still shares match-wide HUD elements and uses one gamepad plus keyboard. The internal 1280x720 canvas now scales into a resizable window or borderless fullscreen with letterboxing. Native high-resolution rendering, general controller rebinding and independent accessibility scaling remain opportunities. Independent audio and camera-shake settings are implemented.
...
## September 25 user play-test corrections (0.13)

The user's hands-on report identified five high-impact problems that the prior automated suite had not adequately covered. These changes preserve modes, settings version 3, ghosts, protocol v4 and the Windows headless release gate.

| Priority | Confirmed problem and source | Implemented correction |
| --- | --- | --- |
| P1 | The window lacked resize/fullscreen controls. `Game::Impl` initialization, `draw`, `handleGlobalInput` in [Game.cpp](../src/Game.cpp). | Resizable window (minimum 640x360), F11/Alt+Enter borderless toggle, and an aspect-preserving 1280x720 render canvas for all menus/HUD/split-screen. Toggle input is consumed before menu actions; F11 is reserved during binding assignment/profile validation. |
| P1 | A/D and aerial/dodge direction were reversed from the driver's view; previous tests checked the wrong world-angle convention. `driveCar`, `aiControls`, `directionalDodge`. | Correct ground and aerial yaw and lateral dodge direction; corresponding AI steering signs retain world-space navigation. Tests now use camera-right vectors at four headings, including the opposite end of the court. |
| P1 | The server's contact cancelled receiver ownership; receiving duty always selected P1, support crossed the receiver's approach, and coasting could still cause rear-end collisions. `resetRound`, `registerTeamTouch`, `updateTactics`, `aiControls`; `planTeam`/`avoidTeammates` in [GameplayLogic.cpp](../src/GameplayLogic.cpp). | Rotate first receiver, preserve that priority until the receiving team touches, cover behind/beside the receiver, penalize immediate repeat commitments, and allow an available partner to rescue. Every bot checks teammates each physics tick, detours, brakes against momentum and suppresses boost/jump/dodge while yielding. HUD explains first-touch ownership and partner intent. |
| P1 | High-lift contacts could travel beyond the opposite court before descending. `checkBallImpact`, `controlledVolleyVelocity`. | After real contacts, compress excess horizontal range using flight time and available court distance. Preserve horizontal aim and vertical velocity, apply equally to humans/bots on both difficulties, and leave moderate shots and downward spikes unchanged. Out-of-bounds faults remain possible; no target seeking or free-flight correction was added. |
| P2 | Outdoor scenery and cage geometry could obscure the player/ball; alpha alone still allowed depth writes to hide later objects. `sceneryOpacity`, `drawSceneryCube`, `drawArena`, `drawWorld`. | Test each viewport's camera-to-player and camera-to-ball sightlines, fade blocking exterior cubes to 10% opacity and suppress their depth writes. Net/court geometry remains readable. Replay visibility uses sampled positions. |

The real-Jolt teammate-blocking regression initially failed with detouring/coasting alone; active braking fixed the reproduced collision. Regression coverage now checks rotating receives, server-contact ownership, unavailable-receiver rescue, second-touch reassignment, momentum-aware avoidance, camera-relative controls, and raw-versus-controlled clears. Real Jolt clears land on the opposing floor on Rookie and Pro. Four seeded 60-second 2v2/3v3 matches still pass stability/progress/exchange assertions.

Final 0.13 verification: Windows Release build succeeded; **6/6 CTests passed in 24.51 seconds**, including the local graphical smoke test; the explicit dependency-free configuration passed **3/3 in 2.57 seconds**. The portable ZIP passed checksum, layout, adjacent-runtime and tested-executable verification. Its extracted executable passed both `--protocol-test` and `--headless-test` with exit code 0. The smoke test resized the actual window to 960x800 and checked the resizable flag. Resized and split-screen foreground-obstruction screenshots were inspected; the HUD remains inside the letterboxed canvas and player/ball are visible through foreground scenery. This is automated rendering and screenshot inspection, not hands-on play. Use cases UC-20 through UC-24 are in [USE_CASES.md](USE_CASES.md).

Manual acceptance still required: F11/Alt+Enter enter/exit on the player's monitor/DPI setup; A/D, reverse, aerial and dodge feel with keyboard/controller; human + AI first/second-touch rallies; hard clears and intentional spikes on both difficulties; moving along trees/stands in both cameras and split-screen. Bots do not yet deliberately construct set/spike combinations or aim passes to a human. Avoidance cannot guarantee no collision in every crowded aerial scramble. Transparency changes immediately rather than fading over time, and the scalable canvas does not add native 4K detail. Clean-machine package launch and hosted Actions execution remain unverified locally.
